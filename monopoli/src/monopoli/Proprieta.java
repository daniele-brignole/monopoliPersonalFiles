...
public class Proprieta extends Casella {
	 ...
	 */
	private Contratto c;
	/**inizializza l'oggetto proprieta con i valori di parametro
	 * 
	 * @param p posizione della proprieta
	 * @param n nome della proprieta
	 * @param valore valore di acquisto della proprieta
	 * @param colore colore della zona di appartenenza della proprieta
	 * @param affitto valore base di affitto della proprieta
	 * @param ipoteca valore di ipoteca della proprieta
	 */
	Proprieta(int p, String n, int valore, int colore,int affitto,int ipoteca) {
		super(p,n);
		posizione = p;
		nome = n;
		this.valore = valore;
	 ...
	 * è finito sulla casella della proprieta a quello che la possiede,
	 * a patto che essa non sia ipotecata. Insieme al metodo calcolaAffitto
	 * si rifa al UC del pagamento dell'affitto
	 * 
	...
	/**metodo per la costruzione delle case su una proprieta posseduta
	 * 
	...
	/**metodo per la costruzione di un albergo su una proprieta con almeno 4 case
	 *  
	...
	/**metodo che controlla se il giocatore possiede tutte le proprieta
	 ...
	 * 		quindi tutte le proprieta di uno stesso colore, false altrimenti
	 * 
	 */
	public boolean isZonaCompleta(Giocatore g){
		...
		ArrayList<Proprieta> myProperties = new ArrayList<Proprieta>(g.getMyProperties());
	...
	/**compra la proprieta libera se si hanno i soldi, si rifa al UC dell'acquisto di una proprieta
	 * 
	 * @param g giocatore che compra
	...
	@Override
	/**Metodo che controlla se la proprieta su cui e finito il giocatore 
	 * sia libera o occupata e ne permette l'acquisto nel primo caso (con possibile rifiuto 
	 * e conseguente messa in asta della proprieta) o
	 * il pagamento dell'affitto nel secondo caso
	 * @param g giocatore che finisce sulla casella di proprieta
	 * 
	 */
	boolean attivaEffetto(Giocatore g) {
		...
		if(!stato) {
			...
			if (risp.equals("s")) compra(g);
				...
				System.out.println("Non hai comprato la proprieta, verra messa all'asta");
